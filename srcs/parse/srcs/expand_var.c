...
/*   Updated: 2023/08/26 07:26:05 by fsuomins         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *replace_variables(char *input_string, t_env_list *env_data)
{
    ...
    char *var_name;
    char *var_value;
    char *final_str = NULL;  // Initialize to NULL to avoid potential issues
    ...
    while (var_head)
    {
		...
		char *temp_str = ft_strjoin(final_str, var_value);
		free(final_str);
...
void expand_variables(t_config *data)
{
    ...
    while (data->tokens)
    {
        ...
        char *expanded_value = replace_variables(data->tokens->value, data->env);
        if (expanded_value)
        {
