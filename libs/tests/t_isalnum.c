...
/*   Updated: 2023/04/22 17:15:47 by fsuomins         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
MU_TEST(test_if_symbols_is_alpha)
{
	...
	{
		if (is_symbol(i))
			mu_assert(ft_isalnum(i) == isalnum(i), "Fail on symbols values");
		i++;
	}
}
...
MU_TEST(test_if_not_ascii_is_alpha)
{
	...
	while (i < 127)
	{
		...
		{
			printf("%d", i);
			mu_assert(ft_isalnum(i) == isalnum(i), "Fail on not ASCII values");
		}
		i++;
	}
}
...
MU_TEST(test_if_spaces_is_alpha)
{
	...
	{
		if (isspace(i))
			mu_assert(ft_isalnum(i) == isalnum(i), "Fail on spaces values");
		i++;
	}
}
