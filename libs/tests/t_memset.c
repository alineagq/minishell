...
/*   Updated: 2023/05/09 20:10:51 by fsuomins         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Test if memset sets all bytes to the given value
...
// Test if memset correctly sets a subset of bytes to the given value
...
// Test if memset works correctly with a size of 0
...
MU_TEST(test_memset_edge_case)
{
    ...
    // Test setting the buffer to all 0x00 bytes using memset
    memset(buffer, 0x00, BUFFER_SIZE);
    mu_assert(memcmp(buffer, "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", BUFFER_SIZE) == 0, "memset failed to set buffer to 0x00");
}
